...
function detectArch() {
    ...
    arm64)
        ...
        ;;
    *)
        echo -n "use default configuration"
        ;;
    esac
}
...
function prepareMaildev() {
    ...
    if [[ $ARCH = "arm32v7" || $ARCH = "arm64v8" ]]; then
        sed -i "/FROM/s/node:/${ARCH}\/node:/g" maildev/Dockerfile
    fi
...
function setMysqlImage(){
    
    if [[ $ARCH = "arm32v7" || $ARCH = "arm64v8" ]]; then
        SQL_IMAGE="jsurf/rpi-mariadb" 
    else 
        SQL_IMAGE="mysql" 
    fi
}
function editDockerCompose() {
    ...
    sed -i "s/SQL_IMAGE/${SQL_IMAGE}/g" docker-compose.yml
}
function initProject() {
    if [ "$IS_NEW_PROJECT" = true ]; then
        if [ "$USER_FRAMEWORK" = 1 ]; then
            docker exec www_dev_env composer create-project --no-interaction -q symfony/website-skeleton $PROJECT_DIR
            sudo chown -R $USER ./
        elif [ "$USER_FRAMEWORK" = 2 ]; then
            docker exec www_dev_env composer create-project --no-interaction -q --prefer-dist laravel/laravel $PROJECT_DIR
            sudo chown -R $USER ./
        fi
    else
    ...
    if [ "$USER_FRAMEWORK" = 1 ]; then
        ...
        sed -i "/DB_HOST=/c\DB_HOST=db_dev_env" $PROJECT_DIR/.env
        sed -i "/DB_DATABASE=/c\DB_DATABASE=${PROJECT_BDD}" $PROJECT_DIR/.env
        sed -i "/MAIL_DRIVER=/c\MAIL_DRIVER=smtp" $PROJECT_DIR/.env
        sed -i "/MAIL_HOST=/c\MAIL_HOST=maildev_dev_env" $PROJECT_DIR/.env
        sed -i "/MAIL_PORT=/c\MAIL_PORT=25" $PROJECT_DIR/.env
        docker exec -i www_dev_env bash -c "cd $PROJECT_DIR  && php artisan key:generate "
    fi
}
